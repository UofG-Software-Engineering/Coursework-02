...
public class Injector {
    ...
    // 已经生成的单例
    private static final Map<Class<?>, Object> finalSingletonMap = Collections.synchronizedMap(new HashMap<>());

    // 准备进行构造的类
    private static final Set<Class<?>> processingInstances = Collections.synchronizedSet(new HashSet<>());
    ...
    private static <T> T createNew(Class<T> clazz) {
    ...
    private static<T> T createFromConstructor(Constructor<T> con) {
     ...
     * 注入 Field
     */
    private static<T> void injectField(T body) {
    ...
    private static <T> void injectMethod(T body) {
    ...
    @SuppressWarnings("unchecked")
    private static<T> T createFromParameter(Parameter parameter) {
        Class<?> clazz = parameter.getType();
        return (T) createNew(clazz);
    }

    @SuppressWarnings("unchecked")
    private static <T> T createFromField(Field field) {
